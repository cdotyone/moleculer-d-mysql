...
class OpenApiGenerator {
	...
	getType(field) {
		return (
			(field.openapi && field.openapi.type
				? field.openapi.type
				: field.type) || "string"
		);
	}

	generateEntityType(target, typeName, fields, kind) {
		const entity = {};
		const requiredList = [];
		...
		entries.forEach(([name, field]) => {
			...
			if (field.primaryKey && kind === "create" && field.generated !== "user")
				return null;
			...
			const type = this.convertType(this.getType(field));
			...
			if (field.type === "array" && field.items) {
				if (field.items.type === "object") {
					// Create a sub-entity
					let subTypeName = `${typeName}${this.capitalize(
						pluralize(name, 1)
					)}`;
					...
					obj.items = { type: this.convertType(field.items.type) };
				}
			}

			if (field.openapi && field.openapi.example)
				obj.example = field.openapi.example;
	...
	convertFVToJsonSchema(def) {
		def = _.isString(def) ? validator.parseShortHand(def) : def;

		if (Array.isArray(def)) {
			return { oneOf: def.map(this.convertFVToJsonSchema) };
		}

		const res = {
			type: this.convertType(this.getType(def))
		};

		if (def.type === "array" && def.items) {
			res.items = this.convertFVToJsonSchema(def.items);
		}
		...
		if (def.type === "object" && def.properties) {
			...
			for (const [name, param] of Object.entries(def.properties)) {
				...
				res.properties[name] = this.convertFVToJsonSchema(param);
			}
		}
		return res;
	}

	makeOpenAPIPath(basePath, actionRest) {
		let method;
		let path;

		if (_.isString(actionRest)) {
			const parts = actionRest.split(" ");
			method = parts.shift().toUpperCase();
			path = parts.join("/");
		} else {
			method = actionRest["method"];
			path = actionRest["path"];
		}
	...
	generateActionOpenAPISchema(
		...
		const pluralizedLabel = this.uncapitalize(pluralize(entityName));
		const entityLabel = this.uncapitalize(entityName);

		const res = {
			$path: this.makeOpenAPIPath(basePath, actionDef.rest),
			tags: [tagName],
			operationId: actionFullName
		};

		const hasBody = ["POST", "PUT", "PATCH", "DELETE", "GET"].includes(
			res.$path.method
		);
		res.parameters = [];
		...
		for (const [name, param] of Object.entries(actionDef.params)) {
			...
			const obj = this.convertFVToJsonSchema(param);
		...
		// custom code to support application/xml request 
		if (actionDef.isXMLRequestAllowed) {
			res.requestBody.content["application/xml"] = {
				schema: {
					type: "object",
					properties: body[actionDef.xmlRootElement].properties,
					xml: {
						name: actionDef.xmlRootElement
					}
				}
			};
		}
		if (isDatabaseService) {
			switch (actionName) {
				...
				case "replace": {
					if (!res.summary)
						res.summary = `Update an existing ${entityLabel}`;
					res.responses = {
						200: {
				...
				case "remove": {
					if (!res.summary)
						res.summary = `Remove an existing ${entityLabel}`;
					res.responses = {
						200: {
							description: `Updated ${entityName}`,
							content: {
				...
				case "list": {
					...
					if (!res.summary)
						res.summary = `List ${pluralizedLabel} (with pagination)`;
					res.responses = {
						200: {
							description: `Found ${pluralizedLabel}`,
							content: {
	...
	generateOpenAPISchema(name, schema) {
		const entityName = this.capitalize(pluralize(name, 1));
		const pluralizedEntityName = pluralize(entityName);
		...
		const serviceFullName = Service.getVersionedFullName(
			schema.name,
			schema.version
		);
		...
		const tagName = this.uncapitalize(pluralizedEntityName);
		...
		Object.keys(schema.actions).forEach((actionName) => {
			...
			if (actionDef.rest && visibility === "published" && actionDef.params) {
				actionDef.openapi = _.defaultsDeep(
					actionDef.openapi,
					this.generateActionOpenAPISchema(actionDef, {
						schema,
						basePath,
						entityName,
						actionName,
